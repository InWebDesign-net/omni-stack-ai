...
export default function GlobalUploadManager() {
  ...
  if (isMinimized || (!isManagerOpen && tasks.length > 0)) {
    return (
      <div
        onClick={openManager}
        className="fixed bottom-5 right-5 z-[9999] bg-[#0d1528] border border-[#8083ff]/40 rounded-2xl p-4 shadow-2xl flex items-center gap-3 cursor-pointer hover:border-[#8083ff] transition-all animate-fadeIn"
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Canvas */}
      <div
        className="relative w-full max-w-2xl bg-[#080e1e] border border-white/10 rounded-3xl p-6 shadow-2xl z-50 flex flex-col gap-5 max-h-[90vh] overflow-hidden animate-scaleIn font-sans"
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 rounded-2xl bg-[#162038] border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
              <FileVideo className="h-6 w-6 text-[#8083ff]" />
            </div>
            <div className="w-12 h-12 rounded-2xl bg-[#162038] border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
              <ImageIcon className="h-6 w-6 text-[#44e2cd]" />
            </div>
          </div>

          <h4 className="text-sm font-bold text-white mb-1">
            {t.upload?.dropHere || 'Videos oder Bilder hier hineinziehen'}
          </h4>
          <p className="text-xs text-[#9ba4bf] max-w-sm">
            {t.upload?.dropHint || 'Unterstützt MP4, MOV, MKV, WebM sowie JPG, PNG, WebP (automatische Konvertierung)'}
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3 overflow-y-auto pr-1 max-h-64 custom-scrollbar">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-[#9ba4bf] uppercase tracking-wider">
                {t.upload?.queueLabel || 'Upload-Warteschlange'} ({tasks.length})
              </span>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5 font-medium">
                      {task.status === 'queued' && (
                        <span className="text-[#9ba4bf]">{t.upload?.queued || 'Eingereiht'}</span>
                      )}
