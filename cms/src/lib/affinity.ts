 ...
 * the whole stack (tracking service, feed ranking, AI intent, frontend panel).
 *
 * Shape:
 ...
 *
 * A mirrored copy lives in web/src/lib/affinity.ts — keep both in sync.
 */
...
export function isCanonicalAffinityGraph(raw: any): boolean {
  return Boolean(
    raw &&
      typeof raw === 'object' &&
      raw.topics &&
      ...
      raw.creators &&
      raw.contentTypes &&
      (raw.activePattern === 'discovery' || raw.activePattern === 'deep_dive')
  );
}
...
export function creatorWeight(graph: AffinityGraph, creatorId?: string | number): number {
