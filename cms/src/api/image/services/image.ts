...
export default factories.createCoreService('api::image.image', ({ strapi }) => ({
  /**
   * Native Strapi 5 database filtering & pagination service for Images.
   * Handles locale selection, title search (q), sorting, and tag include/exclude/matchmode
   * returning exact 24-item paginated payloads.
   */
  async findFilteredImages(params: any = {}) {
    ...
    if (includeList.length > 0 || excludeList.length > 0) {
      ...
      if (docIds.length > 0) {
        allLocaleItems = await strapi.documents('api::image.image').findMany({
